...
class Resource {
   ...
   */
  JS(src, callback) {
    this.script("text/javascript", src, callback);
  }
...
class DOM {
  ...
  onclick(id, run) {
    ...
    var client =
      run.client !== undefined ? 'Konekti.client["' + run.client + '"].' : "";
      ...
      switch (state) {
        ...
        case 2:
          if (x[i].length > 0 || i == x.length - 1) {
            ...
            else if (typeof dictionary[tag] === "string")
              res += dictionary[tag];
...
class FontSize {
  ...
  fromFontSize(fs) {
    ...
    if (fs.endsWith("em"))
      return parseFloat(fs.substring(0, fs.length - 2)) * 16;
    return null;
  }
  ...
  size(config) {
    ...
    if (config.style["font-size"] !== undefined)
      return this.fromFontSize(config.style["font-size"]);
    return null;
  }
}
...
class Client {
   ...
   */
  assign(config) {
    for (var x in config) this[x] = config[x];
  }
  ...
  widthResize(width) {
    ...
    switch (x.config.width) {
      ...
      case "fit":
        ...
        break;
      default:
        if (x.config.width.endsWith("px"))
          w = parseFloat(
            x.config.width.substring(0, x.config.width.length - 2)
          );
        else
          w =
            (width *
              parseFloat(
                x.config.width.substring(0, x.config.width.length - 1)
              )) /
            100;
    }
    return w;
  ...
  heightResize(height) {
    ...
    switch (x.config.height) {
      ...
      case "fit":
        ...
        break;
      default:
        if (x.config.height.endsWith("px"))
          h = parseFloat(
            x.config.height.substring(0, x.config.height.length - 2)
          );
        else
          h =
            (height *
              parseFloat(
                x.config.height.substring(0, x.config.height.length - 1)
              )) /
            100;
    }
    return h;
  ...
  computeSize(size, style, fitSize) {
    ...
    if (style.endsWith("%"))
      return (size * parseFloat(style.substring(0, style.length - 1))) / 100;
    return size;
  }
  ...
  resize(width, height) {
    ...
    width = x.computeSize(
      width,
      x.config.width,
      x.parent !== undefined ? Konekti.client[x.parent].fitWidth() : 0
    );
    height = x.computeSize(
      height,
      x.config.height,
      x.parent !== undefined ? Konekti.client[x.parent].fitHeight() : 0
    );
...
class KonektiAPI {
  ...
  resize() {
    ...
    Konekti.daemon(
      function () {
        return x.vc() !== undefined && x.vc() !== null;
      },
      function () {
        x.client["body"].resize(window.innerWidth, window.innerHeight);
      }
    );
  }
   ...
   */
  vc(id = "body") {
    return id == "body" ? document.body : document.getElementById(id);
  }
  ...
  expand(component, callback) {
    ...
    if (
      ...
      component.plugin !== undefined &&
      component.setup !== undefined
    ) {
        ...
        Konekti.daemon(
          ...
          function () {
            ...
            function add_child(child) {
              ...
              while (
                j < c.children.length &&
                (c.children[j].setup === undefined ||
                  child.id != c.children[j].setup[1])
              )
                j++;
              c.children[j] = child;
              k++;
  ...
  raw(id, children = [], config = {}, callback = function () {}) {
    Konekti.add(
      { plugin: "raw", setup: ["body", id, children, config] },
      callback
    );
  }
  ...
  config(json) {
    ...
    if (typeof json.style === "string")
      json.style = Konekti.dom.style(json.style);
